...
- Sử dụng Tailwind vào dự án
  - IntelliSense: tiện ích trong VSCode Prettier (PrettierCode formatter)
  - Tailwind Prettier Plugin (prettier-plugin-tailwindcss)
- Cách Tailwind CSS hoạt động
  - Tailwind quét toàn bộ dự án, tìm các utility class rồi chuyển thành css
- Quy tắc xậy dựng class
  - <property> - <value>
  - value : số / chữ (round[50%])
    Tailwin Cheat Sheet

* MAP VÀ SET TRONG JAVASCRIPT
  - SET: loại bỏ đi giá trị bị trùng
    const number = new Set([1, 2, 3, 2 , 4])
* ES6

- Optional chaining ( ?.)
- Destructuring: Phân rã - const {firstName , age } = persion
- Rest: Phần còn lại - const {firstName , age, ...rest } = persion
